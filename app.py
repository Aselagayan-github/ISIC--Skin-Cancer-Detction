...
def is_valid_skin_image(filename):
    """Check if the image is a valid skin image based on its filename."""
    # For this example, we'll assume valid skin images start with 'ISIC'
    return filename.startswith('ISIC')
...
def showresult():
    ...
    if not is_valid_skin_image(file.filename):
        error_message = "Invalid input: The uploaded image is not recognized as a skin image. Please upload a valid skin image."
